...
public class UserSPManager {
    ...
    private static final String KEY_USER_DATA = "user_data";// 用户信息数据
     ...
     * @param data 登录数据
     */
    public static void putLoginData(LoginBean data) {
        if (data == null) {
    ...
    public static void putUser(UserDto user) {
        if (user == null) {
            getUserInfoSharedPreferences().edit().remove(KEY_USER_DATA).apply();
            return;
        }
        String cipher = CipherUtils.encryptAES(Config.CACHE_PASSWORD, Config.CACHE_SALT, user);
        if (cipher == null)
            return;
        getUserInfoSharedPreferences().edit().putString(KEY_USER_DATA, cipher).apply();
    }
    ...
    public static UserDto getUser() {
        String cipher = getUserInfoSharedPreferences().getString(KEY_USER_DATA, null);
        if (TextUtils.isEmpty(cipher))
            return null;
        return CipherUtils.decryptAES(Config.CACHE_PASSWORD, Config.CACHE_SALT,
                cipher, UserDto.class);
    }
